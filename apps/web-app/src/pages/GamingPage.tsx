...
const GAMES = [
  {
    id: 'coinflip', emoji: '🪙', name: 'Coin Flip',
    desc: 'Pick heads or tails. Double your MUSD instantly.',
    minBet: 1, maxBet: 500, players: 1247, tag: 'Instant',
    color: '#f59e0b', bg: 'rgba(245,158,11,0.1)',
  },
  {
    id: 'predict', emoji: '📊', name: 'Price Prediction',
    desc: 'Will BTC be higher or lower in 1 hour? Bet MUSD.',
    minBet: 5, maxBet: 1000, players: 892, tag: 'Trending',
    color: 'var(--accent)', bg: 'rgba(124,110,247,0.1)',
  },
  {
    id: 'lottery', emoji: '🎰', name: 'MUSD Lottery',
    desc: 'Weekly jackpot. Buy tickets for $1 MUSD each.',
    minBet: 1, maxBet: 100, players: 3201, tag: 'Jackpot',
    color: 'var(--green)', bg: 'rgba(34,211,168,0.1)',
  },
  {
    id: 'crash', emoji: '🚀', name: 'Rocket Crash',
    desc: 'Cash out before the rocket crashes. Multiplier grows in real time.',
    minBet: 2, maxBet: 250, players: 654, tag: 'New',
...
export default function GamingPage() {
  ...
  return (
    <div className="page-content fade-in">
      <TopBar title="Games" blockNumber={live.blockNumber} gasPriceGwei={live.gasPriceGwei} />
      <div className="content-wrap">
        ...
        <div className="metrics-row four-col">
          ...
          <MetricCard label="Total Prizes"   value="$17,500"           sub="In active pools"     changeDir="up" change="Live" />
            ...
            {TOURNAMENTS.map(t => (
              <div key={t.name} style={{ padding: '14px 0', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--muted)' }}>{t.entries} entries · ends in {t.ends}</div>
                </div>
          ...
          <div className="card">
            ...
            {LEADERBOARD.map(p => (
              <div key={p.rank} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                ...
                <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: p.rank <= 3 ? 'var(--text)' : 'var(--sub)' }}>
                  {p.name}
                </span>
